<?php
    ...
    require_once( K_ENGINE_DIR.'field.php' );
    require_once( K_ENGINE_DIR.'folder.php' );
            ...
            if( is_null($this->tpl_id) && is_null($this->tpl_name) ){
                ...
                if( array_key_exists( $tpl_name, $FUNCS->cached_templates ) ){
                    ...
                    if( !count($rs) ){
                        ...
                        $AUTH->check_access( K_ACCESS_LEVEL_SUPERUSER );
                        ...
                        if( !$rs[0]['executable'] && $AUTH->user->access_level < K_ACCESS_LEVEL_SUPERUSER ){
        ...
        function _fill_page_info( $skip_custom_fields ){
            ...
            if( $this->tpl_nested_pages ){
                ...
                if( $this->accessed_via_browser && $CTX->ignore_context!=1 ){
                    ...
                    // 2. Their pretty-URL structure looks like 'folder-view' and that is how KConn::invoke()
                                ...
                                if( strpos($src, $domain_prefix)===0 ){ // process image only if local
                                    ...
                                    if( $src ){
                                        ...
                                        if( $this->tpl_gallery && K_EXTRACT_EXIF_DATA && $f->name=='gg_image' ){
                                            require_once( K_ENGINE_DIR.'includes/phpExifRW/exifReader.inc' );
        ...
        function get_page_view_link(){
            ...
            if( K_PRETTY_URLS ){
                if( $this->tpl_nested_pages ){// nestable pages will have a different URL structure
                    ...
                    $link = $FUNCS->get_pretty_template_link( $this->tpl_name ) . $this->page_name. '-' . $this->id;
                }
            }
            else{
