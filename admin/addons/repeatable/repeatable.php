    ...
    class Repeatable extends KUserDefinedField{
        ...
        function _render( $input_name, $input_id, $extra='', $dynamic_insertion=0 ){
                            ...
                            <tr id="<?php echo $input_id; ?>-<?php echo $row_id; ?>">
                                ...
                                if( is_null($this->rendered_data) ){ // not handling posted data
                                    ...
                                    for( $y=0; $y<count($this->cells); $y++ ){
                                        ...
                                        if( $c->deleted && $AUTH->user->access_level >= K_ACCESS_LEVEL_SUPERUSER && defined('K_ADMIN') ){
            ...
            <script type="text/javascript">
                $(function(){
                    <?php if( $this->stacked_layout ): ?>
                        ENGINE.rrInit('<?php echo $input_id; ?>', <?php echo( intval($this->no_default_row) ); ?>);
                    <?php else: ?>
        ...
        function store_posted_changes( $post_val ){
            ...
            for( $row=0; $row<count($data); $row++ ){
                ...
                for( $y=0; $y<count($this->cells); $y++ ){
                    ...
                    if( $c->deleted && $AUTH->user->access_level >= K_ACCESS_LEVEL_SUPERUSER
					&& defined('K_ADMIN') ){
                        $field_html .= '<div class="k_cell_deleted">&nbsp;</div>';
                    }
                    if( $this->stacked_layout ){
        ...
        static function _render_repeatable_assets(){
            ...
            if( !defined('REPEATABLE_URL') ){
                ...
                ?>
                    ...
                    ENGINE.rrInit = function( field_id, default_row ){
                        var $field = $('#'+field_id);
                    ...
                    ENGINE.t_confirm_delete_row = "<?php echo $FUNCS->t('confirm_delete_row'); ?>";
                    ENGINE.t_no_data_message = "<?php echo $FUNCS->t('no_data_message'); ?>";
                <?php
                $js = ob_get_contents();
                ob_end_clean();
                $FUNCS->add_js( $js );
