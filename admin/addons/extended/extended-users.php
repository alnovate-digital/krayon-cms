    ...
    class KExtendedUsers{
        ...
        function populate_config(){
            ...
            if( file_exists(K_ADDONS_DIR.'extended/config.php') ){
                ...
                die(
                      ...
                      Perhaps you forgot to rename the 'config.example.php' file in '.../addons/extended' to 'config.php'?
                      </h3>"
                   );
            }
        ...
        function &_get_associated_page( &$user, $id_only=0 ){
            ...
            if( $user->id ){ // not a new user
                ...
                if( count($rs) ){
                    ...
                    return;
                    //die( "ERROR: Extended Users module -  please visitx '".$FUNCS->cleanXSS($this->users_tpl)."' as Superuser to define all required fields" );
                }
        ...
        function delete_user_account( &$pg ){
            ...
            if( $FUNCS->is_non_zero_natural($user_id) && $user_id != -1 ){
                ...
                if( $user->access_level <= K_ACCESS_LEVEL_USER ){
        ...
        function hide_fields( &$pg ){
            ...
            if( $pg->tpl_name==$this->users_tpl ){
                ...
                if( !$f ){
                    die( "ERROR: Extended Users module -  please visit '".$FUNCS->cleanXSS($pg->tpl_name)."' as Superuser to define all required fields" );
                }
        ...
        function sync_user_to_page( &$fields, &$errors, &$pg ){
            ...
            if( $pg->tpl_name==$this->users_tpl && !$errors ){
                ...
                if( $user->id == -1 ){
                    $user->fields[3]->store_posted_changes( K_ACCESS_LEVEL_USER );
                }
                if( $date == '0000-00-00 00:00:00' ){
                    if( $user->access_level<K_ACCESS_LEVEL_SUPERUSER ){
        ...
        function _process_activation(){
            ...
            if( !count($rs) ){
                ...
                $AUTH->user->access_level = K_ACCESS_LEVEL_USER +  1; // to allow an unlogged visitor activate his account
                $errors = $user->save();
                if( $errors ){
        ...
        function add_test_login_cookie( &$html, &$PAGE ){
            if( strlen($this->login_tpl) && $PAGE->tpl_name==$this->login_tpl ){
                global $AUTH;
                setcookie( 'k_testcookie', 'Test cookie', 0, $AUTH->cookie_path, null, K_HTTPS ? true : null );
            }
        }
