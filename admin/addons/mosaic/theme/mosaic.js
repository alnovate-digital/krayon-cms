 ...
 */
ENGINE.mosaicModalClose = function() {
    window.KMosaic = null;
    $.magnificPopup.close();
};
 ...
 */
ENGINE.mosaicModalOpen = function() {
    ...
    window.KMosaic = {
        callBack: function( content, pid, update_link ) {
            ...
            // close modal
            ENGINE.mosaicModalClose();
        }
    };
};

ENGINE.mosaicPopover = function( el, field_id ){
    el.popover({
        ...
        content:   function() {
            ...
            $(source).each(function(index){
                ...
                ENGINE.bindPopupIframe( item, ENGINE.mosaicModalOpen, ENGINE.mosaicModalClose, "mosaic-iframe", true );
            });
...
ENGINE.mosaicActions = function( el, field_id ){
    var row = el.closest('tr');
    var edit_icon = row.find('.col-actions .edit-row');
    edit_icon.attr( 'data_mosaic_row', row.attr('id') );
    ENGINE.bindPopupIframe( edit_icon, ENGINE.mosaicModalOpen, ENGINE.mosaicModalClose, 'mosaic-iframe', true );

    var add_icon = row.find('.col-actions .add-row');
    add_icon.attr( 'data_mosaic_row', row.attr('id') );
    ENGINE.mosaicPopover( add_icon, field_id );
}

ENGINE.mosaicInit = function( field_id ){
    var field = $('#'+field_id);
    field.tableGear({addDefaultRow:1, stackLayout:1});

    ENGINE.mosaicPopover( $('#mosaic_hidden_selector_'+field_id), field_id );
    ENGINE.mosaicPopover( $('#'+field_id).find('.col-actions .add-row'), field_id );
}

$( function(){
    ENGINE.bindPopupIframe( ENGINE.el.$content.find( ".mosaic.tableholder .popup-iframe" ), ENGINE.mosaicModalOpen, ENGINE.mosaicModalClose, "mosaic-iframe", true );
});
