<?php
    ...
    define( 'K_ENGINE_VERSION', '2.2.1' ); // Changes with every release
    define( 'K_ENGINE_BUILD', '20200116' ); // YYYYMMDD - do -

    if( file_exists(K_ENGINE_DIR.'config.php') ){
        require_once( K_ENGINE_DIR.'config.php' );
    }
    else{
    ...
    if ( !K_SITE_OFFLINE && !defined('K_ADMIN') && K_USE_CACHE && $_SERVER['REQUEST_METHOD']!='POST' ){
        ...
        foreach( $_COOKIE as $k=>$v ){
            if( preg_match("/^k_engine_\w{11,}/", $k) ){
                $auth = 1;
                break;
            }
        }
        ...
        if( !$auth && !$no_cache ){
            $k_cache_dir = K_ENGINE_DIR . 'cache/';
    ...
    if( !extension_loaded('mysql') ){
        include_once( K_ENGINE_DIR . 'includes/mysql2i/mysql2i.class.php' );
    }

    if ( !defined('K_SITE_DIR') ) define( 'K_SITE_DIR', dirname( K_ENGINE_DIR ) . '/' );
    ...
    if( strpos($_SERVER['SCRIPT_NAME'], 'php.cgi') !== false ){ //unused in the engine
        $_SERVER['SCRIPT_NAME'] = $_SERVER['PHP_SELF'];
    }

    if ( !defined('K_ADMIN_URL') ) define( 'K_ADMIN_URL', K_SITE_URL . basename(K_ENGINE_DIR). '/' );
    if ( !defined('K_ADMIN_PAGE') ) define( 'K_ADMIN_PAGE', '' );
    if ( !defined('K_GMT_OFFSET') ) define( 'K_GMT_OFFSET', +5.5 );
    define( 'K_SYSTEM_THEME_DIR', K_ENGINE_DIR . 'theme/_system/' );
    define( 'K_SYSTEM_THEME_URL', K_ADMIN_URL . 'theme/_system/' );

    require_once( K_ENGINE_DIR.'functions.php' );
    $FUNCS = new KFuncs();

    // Language file
    global $t;
    $t = array();
    require_once( K_ENGINE_DIR.'lang/EN.php' );
    $FUNCS->_t = $t;
    if( K_ADMIN_LANG != 'EN' && file_exists(K_ENGINE_DIR . 'lang/' . K_ADMIN_LANG . '.php') ){
        $t = array();
        require_once( K_ENGINE_DIR . 'lang/' . K_ADMIN_LANG . '.php' );
    ...
    require_once( K_ENGINE_DIR.'db.php' );
    ...
    $_rs = @mysql_query( "select k_value from ".K_TBL_SETTINGS." where k_key='k_engine_version'", $DB->conn );
    if( $_rs && ($_row = mysql_fetch_row( $_rs )) ){
        $_ver = $_row[0];
        if( version_compare(K_ENGINE_VERSION, $_ver, ">") ){ // Upgrade required
            require_once( K_ENGINE_DIR.'upgrade.php' );
        }
    }
    else{
        // Engine not yet installed or is the database corrupt?
        ...
        require_once( K_ENGINE_DIR.'install.php' );
        die();
    }

    require_once( K_ENGINE_DIR.'auth/auth.php' );
    ...
    if( defined('K_UPLOAD_DIR') ){
        ...
        $engine_dir = K_ENGINE_DIR;
        $Config['UserFilesAbsolutePath'] = $engine_dir . 'uploads' . '/';
    ...
    // full boot of core
    require_once( K_ENGINE_DIR.'page.php' );
    require_once( K_ENGINE_DIR.'tags.php' );
    require_once( K_ENGINE_DIR.'route.php' );
    ...
    // addons to 1.3
    define( 'K_ADDONS_DIR',  K_ENGINE_DIR . 'addons/' );
    ...
    require_once( K_ENGINE_DIR.'addons/mosaic/mosaic.php' );
    ...
    if( defined('K_ADMIN_THEME') ){
        ...
        if( $k_admin_theme ){
            ...
            if( is_dir(K_ENGINE_DIR . 'theme/' . $k_admin_theme) ){
                define( 'K_THEME_NAME', $k_admin_theme );
                define( 'K_THEME_DIR', K_ENGINE_DIR . 'theme/' . $k_admin_theme . '/' );
