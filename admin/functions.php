<?php
    ...
    require_once( K_ENGINE_DIR.'parser/parser.php' );
    require_once( K_ENGINE_DIR.'parser/HTMLParser.php' );
    require_once( K_ENGINE_DIR.'search.php' );
    require_once( K_ENGINE_DIR.'event.php' );
    ...
    class KFuncs{
        ...
        function post_process_page(){
            ...
            if( $AUTH->user->access_level >= K_ACCESS_LEVEL_SUPERUSER ){
                $DB->begin();
        ...
        */
            ...
            if( count($rs) ){
                ...
                foreach( $rs as $key=>$val ){
                    ...
                    if( !$val['has_custom_routes'] ){
                        // Page
                        //RewriteRule ^news/test/[^\.\/]*?([0-9]+)$ news/test.php?p=$1 [L,QSA]
                        $rules[] = 'RewriteRule ^'. $val['pretty_name'].'[^\.\/]*?([0-9]+)$ '.$name.'?p=$1 [L,QSA]';
        ...
        function analyze_link( $link ){
            ...
            if( count($rs) ){
                ...
                foreach( $rs as $key=>$val ){
                    ...
                    // 6. page-view
                    $pattern = '#^'.$val['pretty_name'].'[0-9a-z-_]*?([0-9]+)$#i';
                    $replacement = 'masterpage='.$val['name'].'&is_page=1&p=$1';
                    $ret = @preg_replace( $pattern, $replacement, $link2 );
                    if( $ret!=$link2 ){ return $ret; }
        ...
        function invalidate_cache(){
            ...
            // Invalidate cache
            $file = K_ENGINE_DIR . 'cache/' . 'cache_invalidate.dat';
        ...
        function login_header(){ // now used only by the install routine
            $login_title = 'Krayon';
            $logo_src = K_SYSTEM_THEME_URL.'includes/admin/images/logo_light.png';
            if( K_PAID_LICENSE ){
            ...
            <head>
                ...
                <link href="'.K_SYSTEM_THEME_URL.'includes/bootstrap/css/bootstrap.min.css" rel="stylesheet"/>
                <link href="'.K_SYSTEM_THEME_URL.'includes/admin/main.css" rel="stylesheet"/>
                <script src="'.K_ADMIN_URL.'includes/jquery-3.x.min.js"></script>
                <script src="'.K_ADMIN_URL.'includes/jqueryui/1.x/jquery-ui.js"></script>
                <script src="'.K_SYSTEM_THEME_URL.'includes/svg4everybody.min.js"></script>
                <script src="'.K_SYSTEM_THEME_URL.'includes/admin/main.js"></script>
...
OUT;
        ...
        function load_js( $src='' ){
            ...
            if( $src ){
                ...
                $this->scripts[MD5($src)] = $src . $sep . 'v=' . K_ENGINE_BUILD;
            }
        }
        ...
        function load_css( $src='' ){
            ...
            if( $src ){
                $sep = ( strpos($src, '?')===false ) ? '?' : '&';
                $this->styles[MD5($src)] = $src . $sep . 'v=' . K_ENGINE_BUILD;
            }
        }
            ...
            if( is_null($node) ){
                ...
                if( !$is_code ){
                    if( defined('K_SNIPPETS_DIR') ){ // always defined relative to the site
                        ...
                        $base_snippets_dir = K_ENGINE_DIR . 'snippets/';
                    }
        ...
        function get_template_name(){
            ...
            // For this to work, place define( 'K_TEMPLATE_NAME', 'actual_template_name.php' ); before require_once( 'k-admin/cms.php' );
            if ( defined('K_TEMPLATE_NAME') ) return K_TEMPLATE_NAME;

            // Our assumption here is that 'k-admin' folder resides directly in the website
            ...
            if( substr( $tpl, 0, strlen(K_SITE_DIR) ) != K_SITE_DIR ){
                return $this->raise_error( "'k-admin' folder should reside in the main web-site folder" );
            }
        ...
        function json_encode( $value ){
            ...
            if( !class_exists('Services_JSON') ){
                require_once( K_ENGINE_DIR . 'includes/JSON.php' );
            }
        ...
        function gen_js_for_conditional_fields( $return_js=0 ){
            ...
            if( is_array($PAGE->form_dependencies) ){
                if( !class_exists('KCondFields') ){ require_once( K_ENGINE_DIR . 'conditional-fields.php' ); }
