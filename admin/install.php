<?php
    ...
    $k_stmts[] = "INSERT INTO ".K_TBL_USER_LEVELS." (id, name, title, k_level, disabled) VALUES (1, 'superuser', 'Superuser', 10, 0);";
    $k_stmts[] = "INSERT INTO ".K_TBL_USER_LEVELS." (id, name, title, k_level, disabled) VALUES (2, 'admin', 'Administrator', 7, 0);";
    $k_stmts[] = "INSERT INTO ".K_TBL_USER_LEVELS." (id, name, title, k_level, disabled) VALUES (3, 'moderator', 'Moderator', 4, 0);";
    $k_stmts[] = "INSERT INTO ".K_TBL_USER_LEVELS." (id, name, title, k_level, disabled) VALUES (4, 'user', 'User', 2, 0);";
    $k_stmts[] = "INSERT INTO ".K_TBL_USER_LEVELS." (id, name, title, k_level, disabled) VALUES (5, 'visitor', 'Everybody', 0, 0);";

    // Load dump file for importing data
    if( file_exists(K_ENGINE_DIR . 'install-ex.php') ){
        require_once( K_ENGINE_DIR . 'install-ex.php' );
    }

    function k_install( $name, $pwd, $email ){
        global $CTX, $DB, $FUNCS, $k_engine_tables, $k_stmts;
        $err = '';
        ...
        while( $row = mysql_fetch_row($result) ){
            if( in_array($row[0], $k_engine_tables) ){
        ...
        if( !$err ){
            $k_stmts = array();
            $k_stmts[] = "INSERT INTO ".K_TBL_SETTINGS." (k_key, k_value) VALUES ('k_engine_version', '".K_ENGINE_VERSION."');";
    ...
    <div class="panel-heading simple-heading">Installation Wizard</div>
        ...
        <cms:else />
            ...
            <div class="field prepend">
                <cms:input type="text" id="k_user_name" name="name" maxlength="40"
                    ...
                    autofocus="autofocus" class="text" placeholder="Username" 'required="required"' value=""/>
            ...
            <div class="simple-btns">
                <button class="btn btn-primary btn-block btn-flat" name="k_install" type="submit"><svg class="i"><use xlink:href="<cms:php>echo K_SYSTEM_THEME_URL;</cms:php>assets/open-iconic.svg#check"></use></svg>Install</button>
            </div>
        </cms:if>
    </cms:form>
