<?php
    ...
    class KTags{
        ...
        function editable( $params, $node, $called_from_repeatable=0 ){
            ...
            $lock_file = K_ENGINE_DIR . 'cache/' . '$$'.$PAGE->tpl_id.'-'.$attr['name'];
        ...
        function pages( $params, $node, $mode=0 ){
            ...
            elseif( $mode==2 ){ // called from 'Comments' tag
                ...
                if( $page_name!='' ){
                    //$query_table .= "\n inner join krayon_pages cp on cp.id=cc.page_id";
        ...
        function search_form( $params, $node ){
            ...
            $charset = K_CHARSET;
            $html = <<<FORM
            <form id="k_search_form" class="$form_class" method="get" action="$processor" accept-charset="$charset">
				<input type="text" class="$input_class" name="s" id="s" value="$text" onfocus="if (document.forms['k_search_form'].s.value === '$msg') document.forms['k_search_form'].s.value=''" onblur="if (document.forms['k_search_form'].s.value == '') document.forms['k_search_form'].s.value='$msg'" />
...
FORM;
        ...
        function folder( $params, $node ){
            ...
            if( $AUTH->user->access_level < K_ACCESS_LEVEL_SUPERUSER ) return;
...
MAP;
                ...
                while( !feof($fp) ){
                    ...
                    if( strcmp(trim($res), "VERIFIED")==0 ){
                        ...
                        if( $_POST['payment_status'] == 'Completed' ){
                            ...
                            if( $debug ) $FUNCS->log( 'Krayon validating transaction..', $logfile );
                            $pg = '';
        ...
        // The 'path' is always hard-coded to the site being managed by krayon and so is the domain.
